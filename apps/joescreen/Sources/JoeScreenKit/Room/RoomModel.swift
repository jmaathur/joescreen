...
public struct RoomModel: Codable, Sendable, Equatable {
    ...
    /// RECEIVE-SIDE: record a FOREIGN share learned from a `.shared` ShareEvent or a track
    /// subscription, without bumping `revision` (receiver-local bumps are forbidden — see
    /// `pruneParticipant`). No-op when the window already has an owner (the first attribution
    /// wins; snapshots repair details). Returns whether anything changed.
    @discardableResult
    public mutating func applyForeignShare(_ window: WindowID, owner: ParticipantID,
                                           info: ShareInfo? = nil) -> Bool {
        guard shares[window] == nil else { return false }
        shares[window] = owner
        pauseStates[window] = .live
        if let info { shareInfo[window] = info }
        return true
    }

    /// RECEIVE-SIDE: remove one share (an ordered `.unshared` ShareEvent) and everything hanging
    /// off it, without bumping `revision` (receiver-local bumps are forbidden). Returns whether
    /// anything changed.
    @discardableResult
    public mutating func pruneShare(_ window: WindowID) -> Bool {
        guard shares[window] != nil else { return false }
        shares[window] = nil
        controlModes[window] = nil
        writeAccess[window] = nil
    ...
    /// snapshots, so a stale/reordered snapshot can't resurrect or drop a share. Pause state and
    /// advisory ShareInfo for foreign shares adopt the snapshot's values so those updates keep
    /// flowing. Entries owned by `excludingOwner` (the local participant) are skipped entirely:
    /// that participant is the authoritative source for its own shares, so a stale foreign mirror
    /// can never resurrect a share it just ended or stomp its pause state. Replaces the old
    /// last-writer-wins whole-model replace, whose per-process revision counters collided across
    /// concurrent sharers and silently dropped foreign shares. **NEVER bumps `revision`** — this is
    /// receive-side application, and the receiver-local-bump rule (see `pruneParticipant`) holds.
    @discardableResult
    public mutating func merge(snapshot other: RoomModel, excludingOwner: ParticipantID? = nil) -> Bool {
        ...
        for (window, owner) in other.shares where owner != excludingOwner {
            if shares[window] == nil {
                ...
                shareInfo[window] = other.shareInfo[window]
                changed = true
            } else {
                if let state = other.pauseStates[window], pauseStates[window] != state {
                    pauseStates[window] = state
                    changed = true
                }
                if let info = other.shareInfo[window], shareInfo[window] != info {
                    shareInfo[window] = info
                    changed = true
                }
            }
